...
for (sample in names(sample_dict)){
  ...
  if(sample == 'hs50' | sample == 'hs25' | sample == 'hs35'){
    file_path <- sprintf('../human/%s/output_files/%s/%s_final_%s.tsv', sample, type, sample, type)
...
sample_dict <- list('hs50' = c('human_sperm', 'sperm', 'human_sperm'),
                 'hs25' = c('human_sperm', 'sperm', 'human_sperm'),
                 'hs35' = c('human_sperm', 'sperm', 'human_sperm'),
                 'ht20' = c('human_testis', 'testis', 'human_testis'),
                 'ht55' = c('human_testis', 'testis', 'human_testis'),
                 'ct32' = c('chimp', 'testis', 'chimp'),
                 'ct15' = c('chimp', 'testis', 'chimp'),
                 'gt43' = c('gorilla', 'testis', 'gorilla'),
                 'gt22' = c('gorilla', 'testis', 'gorilla')
                 )
...
plot_data_co <- co_20_mb %>%
  ...
  summarise(count = n()) %>%
  ungroup() %>% 
  complete(bin, species, fill = list(count = 0))
plot_data_co$species <- factor(plot_data_co$species, levels = c('human_sperm', 'human_testis', 'chimp', 'gorilla'))

ggplot(plot_data_co) +
  geom_col(aes(x = bin, y = count, fill = species), position = 'dodge') +
  ggtitle('Number of CO events\nwithin 20 Mb of the telomere') + 
  xlab('Distance to telomere (Mb)') +
  ylab('Count') +
  theme_minimal(base_size = 16) + 
 scale_fill_manual(labels = c('Human sperm', 'Human', 'Chimpanzee', 'Western gorilla'),
                    values = c('#EDDAB9', "#50A3A4",'#FCBC66', '#F97B4F')) +
  theme(legend.position = c(0.85, 0.78),
        legend.title = element_blank())
  #ggsave('../plots/telomere_co_20mb.pdf', width = 190, height = 190, unit = 'mm')
...
plot_data_gcv$species <- factor(plot_data_gcv$species, levels = c('human_sperm', 'human_testis', 'chimp', 'gorilla'))

ggplot(plot_data_gcv) +
  geom_col(aes(x = bin, y = count, fill = species), position = 'dodge') +
  ggtitle('Number of GCV events\nwithin 20 Mb of the telomere') + 
  ylim(0,59) + 
  xlab('Distance to telomere (Mb)') +
  ylab('Count') + 
  theme_minimal(base_size = 16) + 
  ...
  theme(legend.position = c(0.85, 0.78),
        legend.title = element_blank()) 
  #ggsave('../plots/telomere_gcv_20mb.pdf', width = 190, height = 190, unit = 'mm')
...
human_testis_data_co <- rbind(data_frames[['ht20_co']], 
                              data_frames[['ht55_co']])

human_testis_data_gcv <- rbind(data_frames[['ht20_gcv']], 
                              data_frames[['ht55_gcv']])
...
co_calls_with_rr <- rr_in_co_events(human_testis_data_co)
gcv_calls_with_rr <- rr_in_gcv_events(human_testis_data_gcv)

plot_data_co_gcv <- rbind(co_calls_with_rr, gcv_calls_with_rr) %>% group_by(type, rec_rate) %>% summarise(count = n()) 
